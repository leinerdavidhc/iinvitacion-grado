...
const SvgCorner = ({ position }: { position: 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right' }) => {
  const getTransform = () => {
    switch (position) {
      case 'top-left':
        return 'scaleY(-1)';
  ...
  return (
    <motion.div
      className={`absolute w-24 h-24 md:w-32 md:h-32 z-20 text-primary/30 ${getPositionClasses()}`}
    ...
    >
      ...
      >
        <g transform="translate(0,0) scale(1,1)" fill="currentColor" stroke="none">
...
const CornerElements = () => {
  return (
    <>
      <SvgCorner position="top-left" />
      <SvgCorner position="top-right" />
      <SvgCorner position="bottom-left" />
      <SvgCorner position="bottom-right" />
    </>
  );
};
